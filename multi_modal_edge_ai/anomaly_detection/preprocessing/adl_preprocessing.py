...
def dataframe_categorical_to_numeric(df: pd.DataFrame, window_size, one_hot=True) -> Tuple[pd.DataFrame, int]:
    distinct_adl_list = pd.unique(data.iloc[:, 2::3].values.ravel('K'))
...
def window_categorical_to_numeric(window, window_size, adl_encoding, encoding_type):
...
def adl_categorical_to_numeric(adl, adl_encoding, one_hot):
...
def dataframe_standard_scaling(df: pd.DataFrame, n_features):
